...
export default function AboutSection() {
  return (
    <div
      className="bg-brandText min-h-screen flex flex-col justify-center px-6 pt-24 pb-12 sm:pt-32 sm:pb-16 lg:px-8"
      id="about-section"
    >
      <div className="mx-auto max-w-2xl text-center md:mb-10">
        <h2 className="text-white mt-2 text-4xl tracking-tight sm:text-5xl">
          Why us
        </h2>
        <p className="mt-6 text-lg leading-8 text-gray-300 text-balance sm:text-2xl">
      ...
      <div className="mx-auto max-w-7xl divide-y divide-gray-300/10 lg:flex lg:justify-center lg:divide-x lg:divide-y-0 py-8">
        {perks.map((perk, perkIdx) => (
          <div key={perkIdx} className="py-8 lg:w-1/3 lg:flex-none lg:py-0">
            <div className="mx-auto flex max-w-md items-start px-4 lg:max-w-none lg:px-8">
              <perk.icon
                className="h-8 w-8 flex-shrink-0 text-brandPopup"
                aria-hidden="true"
              />
              <div className="ml-4 flex flex-auto flex-col">
                <h3 className="font-medium mb-1 text-white">{perk.name}</h3>
